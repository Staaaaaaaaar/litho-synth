...
def _find_repository_root() -> Path:
    ...
    for starting_point in candidates:
        for candidate in (starting_point, *starting_point.parents):
            if (candidate / "pyproject.toml").is_file() and (candidate / "src" / "rocksynth").is_dir():
                return candidate
    raise RuntimeError("Could not locate the RockSynth repository root")


REPOSITORY_ROOT = _find_repository_root()
sys.path.insert(0, str(REPOSITORY_ROOT / "src"))

from rocksynth.backends.blenderproc import render_scene
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate a calibrated RockSynth scanned-rock scene."
    )
    ...
    print(f"RockSynth scene written to {output_dir}")
